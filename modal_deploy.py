...
def get_opengl_renderer_info():
    output = subprocess.check_output(["glxinfo", "-B"], text=True)
...
def monitor_gpu(output_file, stop_event, interval=1.0):
    opengl_renderer = get_opengl_renderer_info()
    print(f"OpenGL Renderer: {opengl_renderer}")
    timestamp_start = time.time()
    gpu_stats = []
    initial_stats = {
        "opengl_renderer": opengl_renderer,
        "monitoring_start": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(timestamp_start)),
    }
    gpu_stats.append(initial_stats)
        ...
        try:
            ...
            for line in nvidia_smi.strip().split("\n"):
                ...
                if len(parts) >= 9:
                    ...
                    print(f"GPU stats: Util={stats['gpu_utilization']}%, Mem={stats['memory_used']}/{stats['memory_total']}MB, Temp={stats['temperature']}°C")
            time.sleep(interval)
...
def render_manim_gpu(scene_name="SimpleAnimation"):
    ...
    for key, value in gpu_env.items():
        cmd += f"{key}={value} "
    cmd += f"manim -ql --renderer=opengl --write_to_movie binary_search.py {scene_name} --disable_caching"
    ...
    if not found_mp4_files:
        ...
        return {
            ...
            "monitoring_file": monitoring_output if os.path.exists(monitoring_output) else None,
        }
    ...
    for root, dirs, files in os.walk(output_path):
        for file in files:
            ...
            output_files.append({"path": rel_path, "size": size_bytes, "full_path": file_path})
    monitoring_rel_path = os.path.relpath(monitoring_output, output_path) if os.path.exists(monitoring_output) else None
    return {
        "device": "GPU",
        "elapsed_time": elapsed_time,
...
def render_manim_cpu(scene_name="SimpleAnimation"):
    ...
    for key, value in cpu_env.items():
        cmd += f"{key}={value} "
    cmd += f"manim -ql --renderer opengl --write_to_movie binary_search.py {scene_name} -o Scene"
...
def generate_gpu_monitoring_report(monitoring_file_path):
    ...
    try:
        ...
        if not monitoring_data:
            ...
            return None
        gpu_util_values = [float(entry["gpu_utilization"]) for entry in monitoring_data if "gpu_utilization" in entry]
        mem_util_values = [float(entry["memory_utilization"]) for entry in monitoring_data if "memory_utilization" in entry]
        mem_used_values = [float(entry["memory_used"]) for entry in monitoring_data if "memory_used" in entry]
        mem_total_values = [float(entry["memory_total"]) for entry in monitoring_data if "memory_total" in entry]
        mem_free_values = [float(entry["memory_free"]) for entry in monitoring_data if "memory_free" in entry]
        relative_times = [float(entry["relative_time"]) for entry in monitoring_data if "relative_time" in entry]
        report = {
            "samples_count": len(monitoring_data),
            "duration_seconds": monitoring_data[-1]["relative_time"] - monitoring_data[0]["relative_time"] if len(monitoring_data) > 1 else 0,
            "gpu_utilization": {
        ...
        with open(report_path, "w") as f:
            ...
            f.write(f"GPU Utilization (%),{report['gpu_utilization']['avg']:.2f},{report['gpu_utilization']['max']:.2f},{report['gpu_utilization']['min']:.2f}\n")
            f.write(f"Memory Utilization (%),{report['memory_utilization']['avg']:.2f},{report['memory_utilization']['max']:.2f},{report['memory_utilization']['min']:.2f}\n")
            f.write(f"Memory Used (MB),{report['memory_used_mb']['avg']:.2f},{report['memory_used_mb']['max']:.2f},{report['memory_used_mb']['min']:.2f}\n")
            f.write(f"Memory Free (MB),{report['memory_free_mb']['avg']:.2f},N/A,{report['memory_free_mb']['min']:.2f}\n")
        ...
        plt.axhline(y=float(mem_total_values[0]), color='r', linestyle='--', linewidth=1, label=f'Total Memory: {mem_total_values[0]} MB')
...
def main():
    ...
    if "monitoring_file" in gpu_result and gpu_result["monitoring_file"]:
        ...
        if monitoring_report:
            ...
            print(f"Average Memory Usage: {monitoring_report['memory_used_mb']['avg']:.2f} MB ({monitoring_report['memory_utilization']['avg']:.2f}%)")
            print(f"Maximum Memory Usage: {monitoring_report['memory_used_mb']['max']:.2f} MB ({monitoring_report['memory_utilization']['max']:.2f}%)")
    ...
    if gpu_result and cpu_result:
        ...
        if "monitoring_file" in gpu_result and gpu_result["monitoring_file"]:
            ...
            monitoring_report = generate_gpu_monitoring_report.remote(os.path.join(f"/root/output/gpu", gpu_result["monitoring_file"]))
            if monitoring_report:
                print(f"Samples collected: {monitoring_report['samples_count']}")
                print(f"Total Memory: {monitoring_report['memory_total_mb']['value']:.2f} MB")
                print(f"Memory Usage: Avg={monitoring_report['memory_used_mb']['avg']:.2f}MB ({monitoring_report['memory_utilization']['avg']:.2f}%), Max={monitoring_report['memory_used_mb']['max']:.2f}MB ({monitoring_report['memory_utilization']['max']:.2f}%)")
                print(f"Memory Free: Min={monitoring_report['memory_free_mb']['min']:.2f}MB")
                print(f"GPU Utilization: Avg={monitoring_report['gpu_utilization']['avg']:.2f}%, Max={monitoring_report['gpu_utilization']['max']:.2f}%")
